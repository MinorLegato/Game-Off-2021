...
#define GLSL_SHADER(...) "#version 330 core\n" #__VA_ARGS__
...
static void window_keyCallback(GLFWwindow* window, int key, int a, int action, int b) {
    (void)window;
    (void)a;
    (void)b;
...
static void window_charCallback(GLFWwindow* window, unsigned int codepoint) {
    platform.keyboard.is_ascii  = 1;
    platform.keyboard.ascii     = codepoint;
}

static void window_mouseButtonCallback(GLFWwindow* window, int button, int action, int a) {
    (void)window;
    (void)a;
...
static void window_scrollCallback(GLFWwindow* window, double xoffset, double yoffset) {
    (void)window;
...
extern void initPlatform(const char* title, int width, int height, int samples) {
    glfwInit();
    ...
    glfwSetKeyCallback(platform_internal.window, window_keyCallback);
    glfwSetCharCallback(platform_internal.window, window_charCallback);
    glfwSetMouseButtonCallback(platform_internal.window, window_mouseButtonCallback);
    glfwSetScrollCallback(platform_internal.window, window_scrollCallback);
...
extern void updatePlatform(void) {
...
extern f64 getCurrentTime(void) {
    return glfwGetTime();
}
...
extern u32 gl_loadShaderFromFiles(const char *vs, const char *fs, MemoryArena* ma) {
    ...
    char* fs_content = fileReadStr(fs, ma);

    u32 program = gl_createShader(vs_content, fs_content);
